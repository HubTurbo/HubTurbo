...
public class ServiceManager {
	private static final Logger logger = LogManager
			.getLogger(ServiceManager.class.getName());
	...
	protected ServiceManager() {
		...
		pullReqService = new PullRequestService(githubClient);
		model = new Model();
	}
	...
	public void setupAndStartModelUpdate() {
		...
		if (repoId != null) {
			modelUpdater = new ModelUpdater(githubClient, model, issuesETag,
					collabsETag, labelsETag, milestonesETag, issueCheckTime);
	...
	public boolean checkRepository(IRepositoryIdProvider repo)
			throws IOException {
	...
	public HashMap<String, List> getResources(IRepositoryIdProvider repoId)
			throws IOException {
		this.repoId = repoId;
	...
	public Milestone editMilestone(Milestone milestone) throws IOException {
		if (repoId != null) {
			return (Milestone) milestoneService
					.editMilestone(repoId, milestone);
		}
		return null;
	}
	...
	public List<IssueEvent> getFeeds() throws IOException {
		...
		// Pull Requests
		int reqCount = 0;
		List<PullRequest> requests = pullReqService.getPullRequests(
				new SearchRepository(user, repo), IssueService.STATE_OPEN);
		if (requests != null && !requests.isEmpty()) {
			for (PullRequest request : requests) {
				if (request != null) {
					if (reqCount < MAX_FEED) {
						reqCount++;
						IssueEvent pullEvent = new IssueEvent();
						pullEvent.setActor(request.getUser());
						pullEvent.setCreatedAt(request.getCreatedAt());
						pullEvent.setEvent("pullrequest");
						eventList.add(pullEvent);
					} else {
						break;
					}
				}
		...
		for (Collection<IssueEvent> currentPage : iter) {
			if (!currentPage.isEmpty()) {
				for (IssueEvent event : currentPage) {
					if (event != null) {
						IssueEvent fetched = issueService.getIssueEvent(user,
								repo, event.getId());
						if (fetched != null) {
	...
	public Issue editIssue(Issue latest, String dateModified)
			throws IOException {
		if (repoId != null) {
	...
	public CommentUpdateService getCommentUpdateService(int id,
			List<Comment> list) {
		return new CommentUpdateService(githubClient, id, list);
	}

	public Comment createComment(int issueId, String comment)
			throws IOException {
		if (repoId != null) {
			return (Comment) issueService.createComment(repoId,
					Integer.toString(issueId), comment);
		}
		return null;
	}
	...
	public ArrayList<TurboIssueEvent> getEvents(int issueId) throws IOException {
		if (repoId != null) {
			return issueService.getIssueEvents(repoId, issueId)
					.getTurboIssueEvents();
		}
	...
	/**
	 * Gets comments for a issue from GitHub, or returns a cached version if
	 * already present.
	 * 
	 * @param issueId
	 * @return
	...
	private List<Comment> getLatestComments(int issueId) throws IOException {
		if (repoId != null) {
			...
			List<Comment> list = comments.stream().map(c -> {
	...
	public List<Label> setLabelsForIssue(long issueId, List<Label> labels)
			throws IOException {
		if (repoId != null) {
			return labelService.setLabels(repoId, Long.toString(issueId),
					labels);
		}
		return new ArrayList<Label>();
	}
	 ...
	 * issue.
	 * */
	public List<Label> addLabelsToIssue(int issueId, List<Label> labels)
			throws IOException {
		if (repoId != null) {
			return labelService.addLabelsToIssue(repoId,
					Integer.toString(issueId), labels);
		}
		return new ArrayList<Label>();
	}

	public void deleteLabelsFromIssue(int issueId, List<Label> labels)
			throws IOException {
	...
	public void deleteLabelFromIssue(int issueId, Label label)
			throws IOException {
		if (repoId != null) {
			labelService.deleteLabelFromIssue(repoId,
					Integer.toString(issueId), label);
		}
	}

	public boolean setIssueMilestone(int issueId, Milestone milestone)
			throws IOException {
		if (repoId != null) {
			Issue result = issueService.setIssueMilestone(repoId, issueId,
					milestone);
