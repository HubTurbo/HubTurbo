...
public class NeoUITest extends FxRobot {
    
    private static final Logger logger = LogManager.getLogger(NeoUITest.class.getName());
    ...
    public static void setupSpec() {
        try {
            ...
            FxToolkit.setupStage(NeoUITest::handleSetupStage);
    ...
    private static void handleSetupStage(Stage stage) {
    ...
    private static void beforeStageStarts() {
    ...
    public FxRobot type(String text) {
        for (int i = 0; i < text.length(); i++) {
            if (specialCharsMap.containsKey(text.charAt(i))) {
                ...
                if (StringUtils.isNumeric(typed)) typed = "DIGIT" + typed;
                if (!typed.equals("/")) {
